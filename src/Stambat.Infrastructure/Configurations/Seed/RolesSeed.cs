...
public class RolesSeed : IEntityTypeConfiguration<Role>
{
    ...
    {
        Role superAdmin = Role.Create("SuperAdmin", "Full system administration and management.", AuthSeedConstants.RoleIdSuperAdmin);
        superAdmin.CreatedAt = AuthSeedConstants.SeedDateUtc;
        superAdmin.CreatedBy = AuthSeedConstants.SystemUserId;

        Role tenantAdmin = Role.Create("TenantAdmin", "Administration of a specific tenant and its resources.", AuthSeedConstants.RoleIdAdmin);
        tenantAdmin.CreatedAt = AuthSeedConstants.SeedDateUtc;
        tenantAdmin.CreatedBy = AuthSeedConstants.SystemUserId;

        Role merchant = Role.Create("Merchant", "Staff access for scanning and stamping loyalty cards.", AuthSeedConstants.RoleIdMerchant);
        merchant.CreatedAt = AuthSeedConstants.SeedDateUtc;
        merchant.CreatedBy = AuthSeedConstants.SystemUserId;

        Role manager = Role.Create("Manager", "Middle management for specific tenant features and staff management.", AuthSeedConstants.RoleIdManager);
        manager.CreatedAt = AuthSeedConstants.SeedDateUtc;
        manager.CreatedBy = AuthSeedConstants.SystemUserId;

        Role userRole = Role.Create("User", "End-user access for viewing cards and rewards.", AuthSeedConstants.RoleIdUser);
