...
public class SetupTenant : ICommandRoute<SetupTenantCommand>
{
    public static async Task<IResult> RegisterRoute(
        [FromBody] SetupTenantCommand request,
        [FromServices] IMediator mediator,
        [FromServices] IValidator<SetupTenantCommand> validator)
    {
        ...
        SetupTenantCommandResult response = await mediator.Send(request);
        return Results.Ok(
            ApiResponse<SetupTenantCommandResult>.SuccessResponse(response));
    }
}
