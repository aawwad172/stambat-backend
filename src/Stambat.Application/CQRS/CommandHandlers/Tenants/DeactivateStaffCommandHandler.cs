...
public class DeactivateStaffCommandHandler(
    ...
    ILogger<DeactivateStaffCommandHandler> logger,
    IUnitOfWork unitOfWork,
    IUserRepository userRepository)
    : BaseHandler<DeactivateStaffCommand, DeactivateStaffCommandResult>(currentUserService, tenantProviderService, logger, unitOfWork)
{
    private readonly IUserRepository _userRepository = userRepository;

    public override async Task<DeactivateStaffCommandResult> Handle(DeactivateStaffCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return new DeactivateStaffCommandResult("Staff member deactivated for this tenant successfully.");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An error occurred while deactivating staff member: {Message}", ex.Message);
